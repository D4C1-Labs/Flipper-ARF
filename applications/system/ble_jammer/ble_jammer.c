...
#include "hci_test.h"
#include "ble_jammer_icons.h"

#define TAG "ble_jammer"
#define HOLD_DELAY_MS 100
#define CHANNEL_DWELL_MS 2
...
static void do_jam_channel(uint8_t nrf24_ch) {
    int ble_ch;
    if(hci_test_nrf24_to_ble_ch(nrf24_ch, &ble_ch) == 0) {
        hci_test_tx_start(ble_ch, 5);
        furi_delay_ms(CHANNEL_DWELL_MS);
        hci_test_stop();
    }
}
...
static void settings_save(PluginState* state) {
    ...
    storage_simply_mkdir(storage, "/ext/apps_data/ble_jammer");

    if(file_stream_open(stream, "/ext/apps_data/ble_jammer/settings.txt", FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        char buffer[128];
...
static void settings_load(PluginState* state) {
    ...
    if(file_stream_open(stream, "/ext/apps_data/ble_jammer/settings.txt", FSAM_READ, FSOM_OPEN_EXISTING)) {
        ...
        if(file_size > 0 && file_size < 1024) {
            ...
            if(bytes_read == file_size) {
                ...
                while(line != NULL) {
                    if(strstr(line, "bluetooth_jam_method=") != NULL) {
...
static void jam_bluetooth(PluginState* state) {
    while(!state->is_stop) {
        if(state->bluetooth_jam_method == BLUETOOTH_MODE_LIST) {
            for(int i = 0; i < (int)sizeof(bluetooth_channels) && !state->is_stop; i++) {
                do_jam_channel(bluetooth_channels[i]);
            }
        } else if(state->bluetooth_jam_method == BLUETOOTH_MODE_RANDOM) {
            do_jam_channel(furi_hal_random_get() % 81);
        } else if(state->bluetooth_jam_method == BLUETOOTH_MODE_BRUTEFORCE) {
            for(uint8_t ch = 0; ch <= 80 && !state->is_stop; ch++) {
                do_jam_channel(ch);
            }
        }
    }
...
static void jam_drone(PluginState* state) {
    while(!state->is_stop) {
        if(state->drone_jam_method == DRONE_MODE_BRUTEFORCE) {
            for(uint8_t ch = 0; ch <= 125 && !state->is_stop; ch++) {
                do_jam_channel(ch);
            }
        } else if(state->drone_jam_method == DRONE_MODE_RANDOM) {
            do_jam_channel(furi_hal_random_get() % 126);
        }
    }
}
...
static void jam_ble_advertising(PluginState* state) {
    while(!state->is_stop) {
        for(int ch = 0; ch < ble_channels_count && !state->is_stop; ch++) {
            do_jam_channel(ble_channels[ch]);
        }
    }
}
...
static void jam_ble_data(PluginState* state) {
    while(!state->is_stop) {
        for(uint8_t ch = 2; ch <= 80 && !state->is_stop; ch += 2) {
            do_jam_channel(ch);
        }
    }
}
...
static void jam_misc(PluginState* state) {
    while(!state->is_stop) {
        for(uint8_t ch = state->misc_start; ch <= state->misc_stop && !state->is_stop; ch++) {
            do_jam_channel(ch);
        }
    }
}
...
static void render_callback(Canvas* canvas, void* ctx) {
    ...
    if(!state->is_radio_ready) {
        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str_aligned(canvas, 64, 10, AlignCenter, AlignTop, "Radio Status");
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(canvas, 64, 40, AlignCenter, AlignTop, "Initializing BLE radio...");
    }
...
static void handle_menu_input(PluginState* state, InputKey key) {
    ...
    state->selected_setting_item = SETTINGS_ITEM_BLUETOOTH_METHOD;
}
...
static void handle_settings_menu_input(PluginState* state, InputKey key) {
    switch(key) {
        ...
        case InputKeyDown:
            ...
            break;
        case InputKeyLeft:
            if(state->selected_setting_item == SETTINGS_ITEM_BLUETOOTH_METHOD) {
...
int32_t ble_jammer_app(void* p) {
    ...
    state->thread = furi_thread_alloc_ex("BLEJammer", 1024, jam_thread, state);
    state->is_radio_ready = true;
        ...
        if(status == FuriStatusOk && event.type == EVENT_KEY) {
            if(event.input.type == InputTypePress) {
                ...
                case InputKeyUp:
                    ...
                    state->hold_counter = 0;
                    if(!state->is_running && state->is_radio_ready) {
                        if(state->current_menu == MENU_MISC && state->misc_state != MISC_STATE_IDLE) {
