...
/*
 * RollJam private API table. RollJam loads protocol/emulate plugins (.fal) at
 * runtime that import SubGhz library symbols now living inside this app's
 * private SubGhz library. This table (plus the CC1101 presets) is added to the
 * plugin composite resolver so the plugins resolve at load time.
 */
static constexpr auto subghz_app_api_table = sort(create_array_t<sym_entry>(
    ...
    API_METHOD(
        subghz_setting_load,
        void,
        (SubGhzSetting* instance, const char* file_path)),
    API_METHOD(
    ...
    API_METHOD(
        ...
        void,
        (SubGhzWorker* instance, void* context)),
    API_METHOD(
        subghz_worker_set_overrun_callback,
        void,
        (SubGhzWorker* instance, SubGhzWorkerOverrunCallback callback)),
    API_METHOD(
        subghz_worker_set_pair_callback,
        void,
        (SubGhzWorker* instance, SubGhzWorkerPairCallback callback)),
    API_METHOD(
        subghz_worker_start,
        void,
        (SubGhzWorker* instance)),
