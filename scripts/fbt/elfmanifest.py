...
class ElfManifestFlag(IntFlag):
    Default = 0
    # 1 << 0 reserved for InsomniaSafe (unused here)
    ForceXIP = 1 << 1
...
class ElfManifestV1Ext:
    """Extended manifest: appends a single flags byte after the V1 layout.

    Backward compatible — the firmware manifest parser treats the flags byte
    as optional based on the section size, so old FAPs (without the byte) and
    new FAPs (with it) both load."""
...
def assemble_manifest_data(
    ...
    for flag in app_manifest.flags:
        if flag in ElfManifestFlag.__members__:
